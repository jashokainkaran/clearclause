"""
DistilRoBERTa NLI baseline evaluation for ClearClause.

Inference only. No training, no fine-tuning, no random splitting, no
modification of the source CSV data or labels.

Model:
  cross-encoder/nli-distilroberta-base  (already NLI-fine-tuned upstream)
...
Outputs (written to outputs/distilroberta_nli/ only):
...
OUTPUT_DIR = OUTPUT_ROOT / "distilroberta_nli"
