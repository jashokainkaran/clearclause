"""
Frozen-test evaluation for the RETUNED fine-tuned DeBERTa-v3-base ClearClause
NLI checkpoint. Identical in structure to evaluate_finetuned_deberta_v3_base.py
-- only the checkpoint path and output folder differ, pointing at the
retuned run's output instead of the first run's.

Why a retuned run exists: the first fine-tune of
MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli (finetuning/finetuned_deberta_v3_base,
EPOCHS=4/LEARNING_RATE=2e-5/WEIGHT_DECAY=0.01) improved in-domain but
regressed out-of-domain BELOW its own zero-shot score on external_test v2
(contradiction recall 0.628 zero-shot -> 0.581 fine-tuned, contradiction F1
0.771 -> 0.704 -- see "DeBERTa-v3-base Fine-Tuning Experiment" in
README.md). finetuning/finetune_deberta_v3_base.py was retuned
(EPOCHS=3, LEARNING_RATE=1e-5, WEIGHT_DECAY=0.05) to try to fix that, and
writes to finetuning/finetuned_deberta_v3_base_retuned/ instead of
overwriting the first run's output, so both can be compared. This script
evaluates that retuned output.

Inference only. No training, no fine-tuning, no random splitting, no
modification of the source CSV data or labels.

Model:
  finetuning/finetuned_deberta_v3_base_retuned  (MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli,
  fine-tuned on train v2 + validation v2 with the retuned hyperparameters --
...
Outputs (written to outputs/run_<timestamp>/finetuned_deberta_v3_base_retuned/):
...
Compare this script's summary_metrics.csv against:
  - evaluate_finetuned_deberta_v3_base.py's (the FIRST v3-base run) to see
    whether the retuned hyperparameters fixed the out-of-domain regression
  - evaluate_finetuned_deberta.py's (the fine-tuned nli-deberta-v3-small run)
    to see whether the retuned v3-base run finally beats it
...
FINETUNED_MODEL_DIR = BASE_DIR / "finetuning" / "finetuned_deberta_v3_base_retuned"
...
OUTPUT_DIR = OUTPUT_ROOT / "finetuned_deberta_v3_base_retuned"
...
def main():
    if not FINETUNED_MODEL_DIR.exists():
        print(
            ...
            "       Run finetuning/finetune_deberta_v3_base.py (with the retuned "
            "hyperparameters) first to produce it.",
            file=sys.stderr,
        )
        sys.exit(1)
