"""
RoBERTa-large-MNLI NLI baseline evaluation for ClearClause.

Inference only. No training, no fine-tuning, no random splitting, no
modification of the source CSV data or labels.

Model:
  FacebookAI/roberta-large-mnli  (already NLI-fine-tuned upstream)
  Loaded via Hugging Face Transformers + PyTorch, forced to LOCAL CACHE ONLY
  (local_files_only=True). Runs on CPU.

  WARNING: this is a "large" checkpoint (~1.4 GB) and noticeably heavier on
  CPU RAM/time than the small/base/distil checkpoints in this project. The
  default BATCH_SIZE below is deliberately small (1) to keep peak memory
  down; raise it only if the machine has headroom.
...
Outputs (written to outputs/roberta_large_mnli/ only):
...
# BATCH_SIZE is only a memory/throughput setting (small here because
# roberta-large is heavy on CPU memory); it is never used to report or
# evaluate speed. MAX_LENGTH is the token-length ceiling checked (without
...
OUTPUT_DIR = OUTPUT_ROOT / "roberta_large_mnli"
