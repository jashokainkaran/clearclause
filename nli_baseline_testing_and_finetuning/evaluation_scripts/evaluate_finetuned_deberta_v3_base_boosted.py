"""
Frozen-test evaluation for the "boosted" fine-tuned DeBERTa-v3-base
ClearClause NLI checkpoint. Identical in structure to
evaluate_finetuned_deberta_v3_base.py / evaluate_finetuned_deberta_v3_base_retuned.py
-- only the checkpoint path and output folder differ.

Why this run exists (third attempt at fine-tuning MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli):
  1st attempt (finetuning/finetuned_deberta_v3_base, EPOCHS=4/LR=2e-5/WD=0.01/
  BOOST=1.5): improved in-domain but regressed out-of-domain BELOW its own
  zero-shot score on external_test v2 (contradiction recall 0.628 zero-shot ->
  0.581 fine-tuned, contradiction F1 0.771 -> 0.704).
  2nd attempt "retuned" (finetuning/finetuned_deberta_v3_base_retuned,
  EPOCHS=3/LR=1e-5/WD=0.05/BOOST=1.5): hypothesised the 1st attempt overfit,
  so trained more conservatively. This made contradiction recall WORSE
  everywhere (internal_test 0.957->0.878, external_test 0.581->0.488) while
  entailment recall rose slightly -- the opposite of what overfitting would
  predict, suggesting the real issue is the model not being pushed hard
  enough toward contradiction, not being pushed too hard overall.
  3rd attempt "boosted" (finetuning/finetuned_deberta_v3_base_boosted,
  EPOCHS=4/LR=2e-5/WD=0.01 -- back to the 1st attempt's values -- with only
  CONTRADICTION_BOOST raised from 1.5 to 2.0): isolates that one variable to
  test the revised hypothesis directly. This script evaluates that output.

Inference only. No training, no fine-tuning, no random splitting, no
modification of the source CSV data or labels.

Model:
  finetuning/finetuned_deberta_v3_base_boosted  (MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli,
  fine-tuned on train v2 + validation v2 with CONTRADICTION_BOOST=2.0 --
  see finetuning/finetune_deberta_v3_base.py)
...
Why the v2 test files, specifically (unlike evaluate_deberta_v3_base.py,
which evaluates the zero-shot base checkpoint on the ORIGINAL files):
  ...
  fine-tuning for per-epoch checkpoint selection, so a score on it reflects
  early-stopping bias, not generalisation -- it is not a fair test-set number.
...
Outputs (written to outputs/run_<timestamp>/finetuned_deberta_v3_base_boosted/):
  <set>_predictions.csv         all original columns plus gold_label_id,
...
Compare this script's summary_metrics.csv against:
  - evaluate_finetuned_deberta_v3_base.py's (1st attempt) and
    evaluate_finetuned_deberta_v3_base_retuned.py's (2nd attempt) to see
    whether raising CONTRADICTION_BOOST alone helped
  - evaluate_finetuned_deberta.py's (the fine-tuned nli-deberta-v3-small run)
    to see whether any v3-base attempt finally beats it
...
FINETUNED_MODEL_DIR = BASE_DIR / "finetuning" / "finetuned_deberta_v3_base_boosted"
...
# this checkpoint (finetuning/finetune_deberta_v3_base.py's MAX_LENGTH).
BATCH_SIZE = 16
MAX_LENGTH = 512
...
OUTPUT_DIR = OUTPUT_ROOT / "finetuned_deberta_v3_base_boosted"
...
def main():
    if not FINETUNED_MODEL_DIR.exists():
        print(
            ...
            "       Run finetuning/finetune_deberta_v3_base.py (with "
            "CONTRADICTION_BOOST=2.0) first to produce it.",
            file=sys.stderr,
        )
        sys.exit(1)
