"""
Zero-shot / off-the-shelf DeBERTa NLI baseline evaluation for ClearClause.

Inference only. No training, no fine-tuning, no random splitting, no
modification of the source CSV data or labels.

Model:
  cross-encoder/nli-deberta-v3-small  (already NLI-fine-tuned upstream)
...
contradiction/entailment/neutral. This is the same verification every other
model script in this project runs.
...
Outputs (written to outputs/deberta_v3_small/ only):
...
OUTPUT_DIR = OUTPUT_ROOT / "deberta_v3_small"
