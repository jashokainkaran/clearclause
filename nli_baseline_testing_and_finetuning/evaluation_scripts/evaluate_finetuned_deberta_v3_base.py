"""
Frozen-test evaluation for the fine-tuned DeBERTa-v3-base ClearClause NLI
checkpoint. Identical in structure to evaluate_finetuned_deberta.py (the
nli-deberta-v3-small fine-tune's eval script) -- only the checkpoint path and
...
Model:
  finetuning/finetuned_deberta_v3_base  (MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli,
  fine-tuned on train v2 + validation v2 -- see finetuning/finetune_deberta_v3_base.py)
...
which evaluates the zero-shot base checkpoint on the ORIGINAL files):
  ...
  finetuning/finetune_deberta_v3_base.py), so a score on it reflects
...
Outputs (written to outputs/run_<timestamp>/finetuned_deberta_v3_base/):
...
These are the primary, unbiased generalisation numbers for this fine-tuned
...
were never read by finetuning/finetune_deberta_v3_base.py. Compare this
script's summary_metrics.csv against evaluate_finetuned_deberta.py's (the
fine-tuned nli-deberta-v3-small run) and the five zero-shot models'
summary_metrics.csv files to see whether fine-tuning the stronger base
checkpoint beat fine-tuning the smaller one.
...
FINETUNED_MODEL_DIR = BASE_DIR / "finetuning" / "finetuned_deberta_v3_base"
...
OUTPUT_DIR = OUTPUT_ROOT / "finetuned_deberta_v3_base"
...
def main():
    if not FINETUNED_MODEL_DIR.exists():
        print(
            ...
            "       Run finetuning/finetune_deberta_v3_base.py first to produce it.",
            file=sys.stderr,
        )
        sys.exit(1)
